            ...
            _ => {
                ...
                match command {
                    ...
                    ClientCommand::GetState => {
                        let answer: Vec<u8> =
                            handle_answer(&mut stream, ExpectedAnswer::State).unwrap();
                        match answer[0] {
                    ...
                    ClientCommand::GetName => {
                        let answer: Vec<u8> =
                            handle_answer(&mut stream, ExpectedAnswer::Name).unwrap();
                        let name = String::from_utf8(answer).unwrap();
                        println!("Power plug name: {}", name);
                    }
                    ClientCommand::GetConsumption => {
                        let answer: Vec<u8> =
                            handle_answer(&mut stream, ExpectedAnswer::Power).unwrap();
                        let power =
                            f32::from_be_bytes([answer[0], answer[1], answer[2], answer[3]]);
