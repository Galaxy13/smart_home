...
impl Home {
    ...
    pub fn remove_device(&mut self, device_name: &str, room_name: &str) -> Result<(), String> {
        match self.rooms.entry(room_name.to_string()) {
            Entry::Occupied(mut occupied) => {
                occupied.get_mut().remove_device(device_name);
                Ok(())
            }
            Entry::Vacant(vacant) => Err(format!("No room '{}' found", vacant.key())),
        }
    }
}
