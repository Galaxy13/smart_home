...
impl Room {
    ...
    pub fn get_device(&self, device_name: &str) -> Result<&dyn DeviceInterface, String> {
        if self.devices.contains_key(&device_name.to_string()) {
            ...
            Err(String::from("No device contains in this room"))
        }
    }
    ...
    pub fn add_device(&mut self, device_type: Devices, device_name: &str) {
        if self.get_devices_names().contains(&device_name.to_string()) {
        } else {
            match device_type {
                Devices::PowerPlug => self.devices.insert(
                    device_name.to_string(),
                    Box::new(PowerPlug::new(device_name.to_string())),
                ),
                Devices::Thermometer => self.devices.insert(
                    device_name.to_string(),
                    Box::new(Thermometer::new(device_name.to_string(), 0.0)),
                ),
            };
        };
    }
    ...
    pub fn remove_device(&mut self, device_name: &str) {
        match self.devices.entry(device_name.to_string()) {
            Entry::Occupied(occupied) => println!(
                "\nDevice {:?} removed from room {:?}",
                occupied.remove_entry().0,
                self.room_name
            ),
            Entry::Vacant(vacant) => println!(
